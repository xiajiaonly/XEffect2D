...
_XVector2& _XVector2::operator = (const _XVector2I & temp)
{
	x = temp.x;
	y = temp.y;
	return *this;
...
_XVector2::_XVector2(const _XVector2I & temp)
{
	x = temp.x;
	y = temp.y;
}

_XVector2 _XVector2::operator + (const _XVector2& temp) const
{
...
_XVector2 _XVector2::operator / (const _XVector2& temp) const
{
    if(temp.x == 0 || temp.y == 0) return _XVector2(x,y);
    return _XVector2(x / temp.x,y / temp.y);
}

_XVector2 _XVector2::operator + (const float& temp) const
{
...
_XVector2 _XVector2::operator / (const float& temp) const
{
    if(temp == 0) return _XVector2(x,y);
    return _XVector2(x / temp,y / temp);
}

void _XVector2::operator += (const _XVector2& temp)
{
    x += temp.x;
    y += temp.y;
}
...
void _XVector2::operator += (const float & temp)
{
    x += temp;
    y += temp;
}
...
//两点直线公式
int twoPointLine(const _XVector2& lineS,const _XVector2& lineE,double *a,double *b,double *c)//(x-x1)/(x2-x1)=(y-y1)/(y2-y1)
{
...
int getIsInRect(const _XVector2& p,const _XVector2& p1,const _XVector2& p2,const _XVector2& p3,const _XVector2& p4)
{
...
int getIsInRect(const _XVector2& p,const _XVector2 *pR,int pRSum)
{
	if(pR == NULL || pRSum <= 2) return 0;	//输入参数合理性检查
	int crashSum = 0;	//射线相交的次数 
	float temp;
...
//this is the function for XPoint3D
_XVector3::_XVector3(float a,float b,float c)
:x(a)
,y(b)
,z(c)
{
...
//this is the function for XIntPoint
_XVector2I::_XVector2I(int a,int b)
:x(a)
,y(b)
{
}

_XVector2I::_XVector2I()
:x(0)
,y(0)
{
}

_XVector2I& _XVector2I::operator = (const _XVector2 & temp)
{
	x = temp.x;
	y = temp.y;
	return *this;
...
_XVector2I::_XVector2I(const _XVector2 & temp)
{
	x = temp.x;
	y = temp.y;
}

//this is the function for XRect
_XRect::_XRect(float xl,float yt,float xr,float yb)
:left(xl)
,top(yt)
,right(xr)
,bottom(yb)
...
_XRect::_XRect(const _XVector2& leftTop,const _XVector2& rightBottom)
:left(leftTop.x)
,top(leftTop.y)
,right(rightBottom.x)
...
void _XRect::set(float xl,float yt,float xr,float yb)
{
    /*if(xl > xr)
    {
        xChangeTwoSum(&xl,&xr);
...
//this is the function for XIRect
_XIRect::_XIRect(int xl,int yt,int xr,int yb)
:left(xl)
,top(yt)
,right(xr)
,bottom(yb)
...
_XIRect::_XIRect(const _XVector2I& p0,const _XVector2I& p1)
:left(p0.x)
,top(p0.y)
,right(p1.x)
,bottom(p1.y)
...
void _XIRect::set(int xl,int yt,int xr,int yb)
{
/*    if(xl > xr)
    {
        xChangeTwoSum(&xl,&xr);
...
bool _XIRect::isCrash(_XIRect& R0)
{
    _XVector2I tempPoint;
    tempPoint.set(left,top);
    if(R0.isInRect(tempPoint)) return true;
    tempPoint.set(left,bottom);
    if(R0.isInRect(tempPoint)) return true;    
    tempPoint.set(right,top);
    if(R0.isInRect(tempPoint)) return true;
...
void pthread_mutex_init(_XCritical &temp,int sum)
{
}

void pthread_mutex_lock(_XCritical &temp)
{
    temp.Lock();
}

void pthread_mutex_unlock(_XCritical &temp)
{
    temp.Unlock();
}
...
extern int quarticEquation(double a,double b,double c,double d,double e,double *x)
{
