...
void _X2DPacker::updatePutOperate(_XObjectBox &objectBox,_XBasicBox &basicBox,bool needRotate,float positionX,float positionY)
{
	_XVector2 tempSize(0.0f,0.0f);
	if(needRotate == false) tempSize.set(objectBox.m_size.x,objectBox.m_size.y);
...
int _X2DPacker::canPutIn(_XObjectBox &objectBox,_XBasicBox &basicBox)	
{
	...
	{//这个箱子没有装过东西，则直接左上角对齐放置
		...
		{//直接放置就可以放得下
			updatePutOperate(objectBox,basicBox,false,0,0);
		...
		if(m_canRotate == true && basicBox.m_size.x >= objectBox.m_size.y && basicBox.m_size.y >= objectBox.m_size.x)
		{//考虑旋转放入
			updatePutOperate(objectBox,basicBox,true,0,0);
	...
	{//这个箱子被装过东西
		...
		if(m_optimizeLevel == 1)
		{
			...
			for(int k = 0;k < basicBox.m_mayPositionYSum;++ k)
			{
				...
				if((m_canRotate == false && tempPy + objectBox.m_size.y > basicBox.m_size.y)
					|| (m_canRotate == true && tempPy + objectBox.m_size.x > basicBox.m_size.y && tempPy + objectBox.m_size.y > basicBox.m_size.y)) continue;
				for(int i = 0;i < basicBox.m_mayPositionXSum;++ i)
				{
					tempPx = (float)(basicBox.m_mayPositionX[i]);
					if((m_canRotate == false && tempPx + objectBox.m_size.x > basicBox.m_size.x)
						|| (m_canRotate == true && tempPx + objectBox.m_size.y > basicBox.m_size.x && tempPx + objectBox.m_size.x > basicBox.m_size.x)) break;
					...
					if(pointOK != 0)
					{//这个点可以
						updatePutOperate(objectBox,basicBox,false,tempPx,tempPy);
						//由于物件的放入更新新的可能
					...
					{//当前组合不符合要求，考虑旋转
						...
						if(pointOK != 0)
						{//这个点可以
							updatePutOperate(objectBox,basicBox,true,tempPx,tempPy);
							//由于物件的放入更新新的可能
		...
		}else
		{
			for(int i = 0;i < basicBox.m_mayPositionSum;++ i)
			{
				...
				if(pointOK != 0)
				{//这个点可以
					updatePutOperate(objectBox,basicBox,false,basicBox.m_mayPosition[i].x,basicBox.m_mayPosition[i].y);
				...
				{//这个点不行，则考虑旋转
					...
					{//允许旋转
						...
						{//这个点可以
							updatePutOperate(objectBox,basicBox,true,basicBox.m_mayPosition[i].x,basicBox.m_mayPosition[i].y);
