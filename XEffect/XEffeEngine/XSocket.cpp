...
#endif
    m_clientConnectState = new char[MAX_CLIENT];
    m_clientDelayTimes = new char[MAX_CLIENT];
    m_clientSocket = new int[MAX_CLIENT];
    m_getDataBuff = new _xSocketData[MAX_DATABUFF_DEEP];
    m_broadcastBuff = new _xSocketData[MAX_DATABUFF_DEEP];
    m_netFlags = new char[MAX_NET_FLAGS];
...
_xSocket::~_xSocket()
{
	...
	mySleep(100000);
    XDELETE_ARRAY(m_clientConnectState);
    XDELETE_ARRAY(m_clientDelayTimes);
    XDELETE_ARRAY(m_clientSocket);
    XDELETE_ARRAY(m_getDataBuff);
    XDELETE_ARRAY(m_broadcastBuff);
    XDELETE_ARRAY(m_netFlags);
    close(m_mySocket);
}

int _xSocket::getDataCheckKey(int length,char *data) const
{
    char temp = 0;
    int i;
    for(i = 0;i < length;++ i)
...
void _xSocket::dataLockProc(int length,char *data,char lockKey)
{
...
int _xSocket::dataUnlockProc(int length,char *data,char lockKey)
{
...
void _xSocket::dataBale(_xSocketData *temp)
{//打包函数完成的主要任务是数据校验和数据加密
    if(dataProtectionFlag) (*temp).lockKey = random(256);            //生成随机密钥
    else (*temp).lockKey = 0;
...
int _xSocket::dataRelieve(_xSocketData *temp)
{//返回数据解密是否成功
...
int _xSocket::createServer(int port)
{
    ...
    memset(&serverAddr,0,sizeof(serverAddr));                                 //取得本机地址
...
void _xSocket::addAClient(int clientSocket)
{
    //查找一个没有使用的空间
...
#ifdef OS_WINDOWS
DWORD WINAPI _xSocket::acceptThread(void * pParam)
#endif
#ifdef OS_LINUX
...
#ifdef OS_WINDOWS
DWORD WINAPI _xSocket::broadcastThread(void * pParam)
#endif
#ifdef OS_LINUX
...
#endif
{
    ...
    char *broadcastServerInfo = NULL;
    try
    {
        broadcastServerInfo = new char[BCServerInfoDataLength];
            ...
            while(1)
            {
                ...
                {//使用UDP广播的形式
                    ...
                    {//队列为空，无需发送
						...
						try
						{
							broadcastUserData = new char[DATA_HEAD_SIZE + (*pPar).m_broadcastBuff[(*pPar).m_broadcastOldPoint].dataLength];
    ...
    XDELETE_ARRAY(broadcastServerInfo);
...
{   
    ...
    memset(&clientAddr,0,sizeof(clientAddr));             //取得本机地址
...
void _xSocket::distroyClient()
{
    m_clientExitFlag = 1;
    mySleep(500000);                   //等待半秒钟，等待所有线程资源释放       
...
int _xSocket::connectToServer(sockaddr_in serverAddr)
{//连接到服务器
#ifdef OS_WINDOWS
    if(initSocket() == 0) return 0;
#endif
...
void _xSocket::setHostID(const char *p)
{
...
int _xSocket::searchNet(const char *p)
{
    setHostID(p);
#ifdef OS_WINDOWS
...
//    mySleep(tempRandTime * 500000)
   
    return m_myConnectState;
}

int _xSocket::searchNet()
{
#ifdef OS_WINDOWS
    if(CreateThread(0,0,searchNetThread,this,0,&m_clientGetBroadcastThreadP) == 0)
#endif
...
#ifdef OS_WINDOWS
DWORD WINAPI _xSocket::searchNetThread(void * pParam)
#endif
#ifdef OS_LINUX
...
#endif
{
    ...
    sockaddr_in addrUDP;
    memset(&addrUDP,0,sizeof(addrUDP));       
    _xSocketData tempBroadCastData;
    tempBroadCastData.pData = NULL;
    try
    {
        tempBroadCastData.pData = new char[BC_DATA_SIZE - DATA_HEAD_SIZE];
        if(tempBroadCastData.pData == NULL)
        {//内存分配失败
            return 0;
        }
    }catch(...)
...
#endif
        int nRecvLength = 0;
        char szRecvBuffer[BC_DATA_SIZE] = { 0 };
            ...
            if(nRecvLength > 0)
            {
                ...
                {//已经连接网络
                    ...
                    {//收到服务器端的广播信号
                        ...
                        try
                        {
                            (*pPar).m_getDataBuff[(*pPar).m_getDataNewP].pData = new char[tempBroadCastData.dataLength *sizeof(char)];
    ...
    XDELETE_ARRAY(tempBroadCastData.pData);
...
#ifdef OS_WINDOWS
DWORD WINAPI _xSocket::mutualThread(void * pParam)
#endif                       
#ifdef OS_LINUX
...
#endif                       
{
   _xSocket *pPar = (_xSocket *) pParam;
    ...
    try
    {
        temp_data.pData = new char[4*sizeof(char)];
...
int _xSocket::getAMessageClient(_xSocketData tempData)
{
...
#ifdef OS_WINDOWS
DWORD WINAPI _xSocket::getDataThreadOnClient(void * pParam)
#endif
#ifdef OS_LINUX
void *_xSocket::getDataThreadOnClient(void * pParam)
#endif
{
    _xSocketData temp_data;
...
int _xSocket::pushABroadcastMessage(_xSocketData tempData)
{
    ...
    XDELETE_ARRAY(m_broadcastBuff[m_broadcastNewPoint].pData);

    try
    {
        m_broadcastBuff[m_broadcastNewPoint].pData = new char[tempData.dataLength];        //分配内存空间
        if(m_broadcastBuff[m_broadcastNewPoint].pData == NULL)
        {
            return 0;    //注意这里是一个非常严重的错误，将会造成非常严重的影响
        }
    }catch(...)
    {
        return 0;
...
int _xSocket::pushAMessage(_xSocketData tempData)
{
    //将数据推入接收队列
    ++ m_getDataNewP;
    ...
    XDELETE_ARRAY(m_getDataBuff[m_getDataNewP].pData);
    //拷贝数据头
    memcpy(&(m_getDataBuff[m_getDataNewP]),&tempData,DATA_HEAD_SIZE);
    //拷贝数据体
    try
    {
        m_getDataBuff[m_getDataNewP].pData = new char[(tempData.dataLength * sizeof(char))];
...
int _xSocket::getAMessageServer(_xSocketData tempData,int clientID)
{
...
void _xSocket::broadcastDataByTcpIP(_xSocketData tempData)
{
...
#ifdef OS_WINDOWS
DWORD WINAPI _xSocket::getDataThreadOnServer(void * pParam)
#endif
#ifdef OS_LINUX
void *_xSocket::getDataThreadOnServer(void * pParam)
#endif
{
...
int _xSocket::sendDataToNet(int clientOrder,char *p,int length)
{
//  m_logFile.addLogInfo("发送数据");
    ...
    {//服务器
        ...
        if(clientOrder >= 0)
		{
			...
			try
			{
				sendData = new char[(length * sizeof(char))];    //分配内存空间来暂存数据
			...
			XDELETE_ARRAY(sendData);//发送完成之后将暂存的数据空间释放
...
int _xSocket::popAMessage(_xSocketData *tempData)
{
    pthread_mutex_lock(m_mutex);
    if(m_getDataOldP != m_getDataNewP)
    {
        ...
        try
        {
            tempData->pData = new char[tempData->dataLength * sizeof(char)];
        ...
        XDELETE_ARRAY(m_getDataBuff[m_getDataOldP].pData);//删除暂存的内存空间
        pthread_mutex_unlock(m_mutex);
        return 1;
    }else
    {
...
int _xSocket::getDataFromNet(int *clientOrder,int *clientID,char **p,int *length)
{
    ...
    {//服务器
        ...
        {//提取消息成功
            ...
            {//数据符合要求之后，将数据传出去
                ...
                XDELETE_ARRAY((*p));//释放p已经有的资源
                try
                {
                    (*p) = new char[tempData.dataLength * sizeof(char)];    //重新给p分配资源
                    if((*p) == NULL) 
                    {
                        XDELETE_ARRAY(tempData.pData);//删除暂存资源
                        return 0;
                    }
                }catch(...)
                {
...
//下面为对外接口
int _xSocket::connectToNet(int port,const char *serverID)
{
...
int _xSocket::xCreateServer(const char *serverIP,int port)
{
...
int _xSocket::xCreateClient(const char *serverIP,int serverPort)
{
    setBroadCastWay(BROADCAST_WAY_TCPIP);
...
int _xSocket::recvDataFunction(int sock,char *temp_data,int length)
{
...
//int _xSocket::sendDataFunction(int sock,char *temp_data,int length)
int _xSocket::sendDataFunction(int sock,_xSocketData temp_data,int length)
{
    ...
    try
    {
        sendData = new char[((DATA_HEAD_SIZE + temp_data.dataLength) * sizeof(char))];    //为数据发送创建存储空间
        if(sendData == NULL) 
		{
			return 0;
		}
    ...
    //test_error = send(sock,sendData,length,0);
    XDELETE_ARRAY(sendData);
    //addLogText(this,"退出数据发送函数!\n","Exit data send proc.\n");
    return test_error;
}

int _xSocket::recvDataProc(int sock,_xSocketData *temp_data)
{
    if(sock < 0) return 0;
    int test_error = 0;
    ...
    {//读取完整包头之后,读取完整的数据部分
        ...
        try
        {
            temp_data->pData = new char[(temp_data->dataLength * sizeof(char))];    //分配内存空间
            if(temp_data->pData == NULL) return 0;
        }
        catch(...)
        {
        ...
        if(test_error > 0 && test_error < temp_data->dataLength)
        {
            ...
            try
            {
                test_data = new char[(temp_data->dataLength * sizeof(char))];
...
void _xSocket::clearDataBuff()
{
    ...
    for(i = 0;i < MAX_DATABUFF_DEEP;++ i)
    {
        ...
        XDELETE_ARRAY(m_getDataBuff[i].pData);
    }
    m_getDataNewP = 0;
    m_getDataOldP = 0;   
    ...
    for(i = 0;i < BC_BUFF_DEEP;++ i)
    {
        ...
        XDELETE_ARRAY(m_broadcastBuff[i].pData);
    }
    m_broadcastNewPoint = 0;
    m_broadcastOldPoint = 0;   
}
/*
int _xSocket::broadcastData(char *pData,int length,int source)        //发送广播数据
{
    //数据和理性检查
    ...
    {//服务器直接将数据推入发送队列
        ...
        try
        {
            m_broadcastBuff[m_broadcastNewPoint].pData = new char[length];        //分配内存空间
...
void addLogText(_xSocket *p,char *pCN,char *pEN)
{
#ifdef DEBUG_FOR_CN
