...
private:
	...
	_XVector2 m_initSpeed;	//初始速度
	float m_decreaseSpeed;	//速度衰减量
	float m_gravity;		//重力加速度
	int m_atomSum;			//粒子的数量

	_XEchoParticles *m_atom;//原子
	const _XTexture *m_texture;		//粒子的贴图
//	int m_w;	//贴图的宽
//	int m_h;	//贴图的高

public:
	int init(const _XVector2& position,const _XVector2& initSpeed,float decreaseSpeed,float gravity,int atomSum,const _XTexture *texture);					//初始化
	void reset();				//设置动作开始
	void move(int timeDelay);	//改变
	void draw() const;				//描绘
	void release();
	int getIsEnd()
	{
