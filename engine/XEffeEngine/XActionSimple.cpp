...
XBool XActionSimpleEx::init(XResPos resPos,
	...
	void * pClass)
{
	...
	if (m_funInit != NULL) if (m_funInit(resPos, m_pClass) == 0) return XFalse;
...
XBool XActionSimpleEx::initComplex(XResPos resPos, const char *actionName, int actionSum, ...)	//复合动作的初始化
{
	if (m_isInited) return XTrue;
	m_funIsEnd = NULL;
	m_funMove = NULL;
