...
class _XSoundFmod:public _XSoundCore
{
protected:
	_XSoundFmod() 
		...
		,m_musicChannel(NULL)
	{}
	_XSoundFmod(const _XSoundFmod&);
	_XSoundFmod &operator= (const _XSoundFmod&);
	virtual ~_XSoundFmod() {}
public:
	static _XSoundFmod& GetInstance()
	{
		static _XSoundFmod m_instance;
		return m_instance;
	}
private:
	_XSoundState m_musicState;
	int m_curMusicVolume;	//当前音乐的音量
	int m_musicTimer;
	int m_musicFadeTime;
	std::vector<_XSoundFadeData> m_soundFadeList;	//队列
