...
private:
	...
	//下面这个是内部调用的方法
	void setPosition(const XVec2& position);	//设置logo显示的位置
public:
	XBool init(const XVec2& position,XResPos resPos,XBool mode = true);		//初始化
	void move(float timeDelay);			//显示logo的动画
	void reset();						//重置
	void draw();						//描绘logo
	XEngineLogo();						//构造函数
	~XEngineLogo();						//构造函数
	void setBackSize(const XVec2& size);		//设置背景图的尺寸
//	void setBackSize(float x,float y);		//设置背景图的尺寸

	//下面这两个方法尚未实现
	void setSize(const XVec2& size);			//设置logo显示的尺寸
	XBool release();	//释放logo的资源
	XBool getIsEnd();
};
inline void XEngineLogo::setPosition(const XVec2& position)	//设置logo显示的位置
{
	m_position = position;
}
