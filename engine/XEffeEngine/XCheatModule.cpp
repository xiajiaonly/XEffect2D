...
bool XCheatModule::init()
{
	...
	m_cheatEdt = XMem::createMem<XEdit>();
	if (m_cheatEdt == nullptr) return false;
	...
	m_cheatMltTxt = XMem::createMem<XMultiText>();
	if (m_cheatMltTxt == nullptr) return false;
	m_cheatMltTxt->initWithoutSkin(XVec2(512.0f, 1024.0f), "Msg");
...
void XCheatModule::commandProc(const char *command)
{
	...
	{
		if (m_cmdProcs[i].eventProc != NULL && 
			m_cmdProcs[i].eventProc(m_cmdProcs[i].pClass, command, retStr))
		{
...
XBool XCheatModule::keyboardProc(int keyOrder, XKeyState keyState)
{
	...
	case XKEY_LCTRL:
	case XKEY_RCTRL:
		m_ctrlBtnDown = (keyState == KEY_STATE_DOWN);
		break;
	case XKEY_LALT:
	case XKEY_RALT:
		m_altBtnDown = (keyState == KEY_STATE_DOWN);
		break;
	case XKEY_X:
...
void XCheatModule::release()
{
	if (!m_isInited) return;
	XMem::XDELETE(m_cheatEdt);
	XMem::XDELETE(m_cheatMltTxt);
	m_isInited = false;
}
}