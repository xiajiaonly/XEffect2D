...
namespace XE{
namespace XDES{
int initialKeyPermutaion[] = {57, 49,  41, 33,  25,  17,  9,
								 1, 58,  50, 42,  34,  26, 18,
								10,  2,  59, 51,  43,  35, 27,
								19, 11,   3, 60,  52,  44, 36,
...
int initialMessagePermutation[] =	   {58, 50, 42, 34, 26, 18, 10, 2,
...
int subKeyPermutation[] =    {14, 17, 11, 24,  1,  5,
								 3, 28, 15,  6, 21, 10,
								23, 19, 12,  4, 26,  8,
								16,  7, 27, 20, 13,  2,
								41, 52, 31, 37, 47, 55,
...
int messageExpansion[] =  {32,  1,  2,  3,  4,  5,
							 4,  5,  6,  7,  8,  9,
							 8,  9, 10, 11, 12, 13,
							12, 13, 14, 15, 16, 17,
							16, 17, 18, 19, 20, 21,
...
int rightSubMessagePermutation[] =    {16,  7, 20, 21,
									29, 12, 28, 17,
									 1, 15, 23, 26,
									 5, 18, 31, 10,
									 2,  8, 24, 14,
...
int finalMessagePermutation[] =  {40,  8, 48, 16, 56, 24, 64, 32,
...
void generateSubKeys(unsigned char* main_key, key_set* key_sets) 
{
	...
	for (i=0; i<56; i++) 
	{
		shift_size = initialKeyPermutaion[i];
	...
	for (i=1; i<17; i++) 
	{
		...
		for (j=0; j<48; j++) 
		{
			shift_size = subKeyPermutation[j];
			if (shift_size <= 28) 
			{
				shift_byte = 0x80 >> ((shift_size - 1)%8);
...
void processMessage(unsigned char* message_piece, unsigned char* processed_piece, key_set* key_sets, int mode) 
{
	int i, k;
	int shift_size;
	...
	for (i=0; i<64; i++) 
	{
		shift_size = initialMessagePermutation[i];
		shift_byte = 0x80 >> ((shift_size - 1)%8);
	...
	for (k=1; k<=16; k++) 
	{
		...
		for (i=0; i<32; i++) 
		{
			shift_size = rightSubMessagePermutation[i];
			shift_byte = 0x80 >> ((shift_size - 1)%8);
	...
	for (i=0; i<64; i++) 
	{
		shift_size = finalMessagePermutation[i];
		shift_byte = 0x80 >> ((shift_size - 1)%8);
