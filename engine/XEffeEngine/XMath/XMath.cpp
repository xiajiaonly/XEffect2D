...
namespace XMath
{
	...
	//对任意序列的无旋转矩形进行顺时针排序
	void vector4Sort(XVector2* v,XVector2* u)
	{
		XBool isChange;
		for(int j = 0;j < 3;++ j)
		{
			...
			for(int i = 0;i < 3;++ i)
			{
				if(v[i].x > v[i + 1].x)
				{
					XMath::swapTwoSum<XVector2>(v[i],v[i + 1]);
					XMath::swapTwoSum<XVector2>(u[i],u[i + 1]);
					isChange = XTrue;
				}
			}
			if(!isChange) break;
		...
		if(v[2].y < v[3].y)
		{
			XMath::swapTwoSum<XVector2>(v[2],v[3]);
			XMath::swapTwoSum<XVector2>(u[2],u[3]);
		}
	}
	//4个点必须要是已经排序了的
	int getEdgePoint(float angle,XVector2 * points,const XVector2* v,const XVector2* u)
	{
		int sum = 0;
		XVector2 centerV((v[0].x + v[2].x) * 0.5f,(v[0].y + v[2].y) * 0.5f);	//提取必要的参数
		XVector2 centerU((u[0].x + u[2].x) * 0.5f,(u[0].y + u[2].y) * 0.5f);	//提取必要的参数
		float wV = -(v[0].x - v[2].x);
		float hV = -(v[0].y - v[2].y);
		float tempAngle = XVector2(fabs(wV),fabs(hV)).getAngle(); 
	...
	int getEdgePointEx(XRect &rect,XVector2 &point,float angle,float * points)	//角度为12点钟方向开始逆时针[0 - PI]
	{
		//如果点不在矩形内部则直接返回
		if(!rect.isInRect(point)) return 0;
		int ret = 0;
		//三个点
		float stateAngle = PI_HALF - XVector2(point.x - rect.left,point.y - rect.top).getAngle();
		if(angle < stateAngle)
		{//符合三个点的情况
			ret = 3;
		...
		//四个点
		stateAngle = PI_HALF + XVector2(point.x - rect.left,rect.bottom - point.y).getAngle();
		if(angle < stateAngle)
		{//符合四个点的情况
			ret = 4;
			points[0] = point.x;	points[1] = point.y;
		...
		//五个点
		stateAngle = PI_HALF * 3.0f - XVector2(rect.right - point.x,rect.bottom - point.y).getAngle();
		if(angle < stateAngle)
		{//符合五个点的情况
			ret = 5;
			points[0] = point.x;	points[1] = point.y;
		...
		//六个点
		stateAngle = PI_HALF * 3.0f + XVector2(rect.right - point.x,point.y - rect.top).getAngle();
		if(angle < stateAngle)
		{//符合六个点的情况
			ret = 6;
			points[0] = point.x;	points[1] = point.y;
	...
	int getEdgePointEx(XRect &vRect,XRect &uRect,XVector2 &vPoint,float angle,float * vPoints,float * uPoints)	//角度为12点钟方向开始逆时针[0 - PI2]
	{
	...
	int getEdgePointEx(XRect &rect,XVector2 &point,float angle,XVector2 * points)
	{
		//如果点不在矩形内部则直接返回
		if(!rect.isInRect(point)) return 0;
		int ret = 0;
	...
	int getEdgePointEx(XRect &vRect,XRect &uRect,XVector2 &vPoint,float angle,XVector2 * vPoints,XVector2 * uPoints)
	{
		int ret = getEdgePointEx(vRect,vPoint,angle,vPoints);
		if(ret <= 0) return 0;
		for(int i = 0;i < ret;++ i)
	...
	//Ta与Tb是两种媒介的光传导系数
	//计算两个移动物体发生碰撞的时间
	double getTwoLineTime(const XVector2 &L1S,const XVector2 &L1E,const XVector2 &L2S,const XVector2 &L2E,float distance,float timer)
	{
	...
	//计算一个移动物体与点发生碰撞的时间
	double getLineToPointTime(const XVector2 &L1S,const XVector2 &L1E,const XVector2 &point,float distance,float timer)
	{
		if(timer <= 0) return -2.0;	//数值非法
		if(distance < 0) return -2.0;	//数值非法
		//再进行一次优化
	...
	double getObjectToLineTime(const XVector2 &L1S,const XVector2 &L1E,const XVector2 &L2S,const XVector2 &L2E,float distance,float timer)	//获取两条线段某个距离的最小解
	{
		if(timer <= 0) return -2.0;	//数值非法
		if(distance < 0) return -2.0;	//数值非法
		//算法1
	...
	//	printf("%f\n",r[3]);
	//}
	double getCrashTimerB2B(const XVector2& position1,const XVector2& speed1,const XVector2& a1,
	...
	double getCrashTimerB2P(const XVector2& position1,const XVector2& speed1,const XVector2& a1,
	...
	double getCrashTimerB2L(const XVector2& position1,const XVector2& speed1,const XVector2& a1,
	...
	double getCrashTimerB2L(const XVector2& position1,const XVector2& speed1,const XVector2& a1,const XLine &pL,float distance,float time)
	{
		double Ptx,Pty;
		double A = pL.A;
		double B = pL.B;
		...
		double E = pL.m_data1;
		double r[4];
		XVector2 positionS = pL.getStartPosition();
		XVector2 positionE = pL.getEndPosition();
		double a,b,c,d;
	...
	//点到直线距离的公式
	double distancePointToLine(const XVector2& point,const XVector2& lineS,const XVector2& lineE)//ax+by+c=0 x0,y0 |ax0+by0+c|/√(a^2+b^2)
	{
		double a = 0;
		double b = 0;
		double c = 0;
	...
	double distancePointToLine(const XVector2& point,double a,double b,double c)
	{
		if(a == 0.0 && b == 0.0) return 0.0;
		return abs(a * point.x + b * point.y + c) / sqrt(a * a + b * b);
	}
	//点到直线的垂足公式
	XVector2 curtatePointToLine(const XVector2& point,const XVector2& lineS,const XVector2& lineE)	//设直线方程为ax+by+c=0,点坐标为(m,n)则垂足为((b*b*m-a*b*n-a*c)/(a*a+b*b),(a*a*n-a*b*m-b*c)/(a*a+b*b))
	{
		double a = 0.0;
		double b = 0.0;
		double c = 0.0;
		twoPointLine(lineS,lineE,&a,&b,&c);
		if(a == 0.0 && b == 0.0) return XVector2::zero;
		return XVector2((b * b * point.x - a * b * point.y - a * c) / (a * a + b * b),
			(a * a * point.y - a * b * point.x - b * c) / (a * a + b * b));
	}
	XVector2 curtatePointToLine(const XVector2& point,double a,double b,double c)
	{
		if(a == 0.0 && b == 0.0) return XVector2::zero;
		return XVector2((b * b * point.x - a * b * point.y - a * c) / (a * a + b * b),
			(a * a * point.y - a * b * point.x - b * c) / (a * a + b * b));
	...
	//两直线交点的公式
	XVector2 crossPointTineToLine(const XVector2& line1S,const XVector2& line1E,const XVector2& line2S,const XVector2& line2E)
	{
		double a0 = 0.0;
		double b0 = 0.0;
		double c0 = 0.0;
		...
		if(a0 * b1 - a1 * b0 == 0.0) return XVector2::zero;
		return XVector2((b0 * c1 - b1 * c0)/(a0 * b1 - a1 * b0),
			(a1 * c0 - a0 * c1)/(a0 * b1 - a1 * b0));
	}
	XVector2 crossPointTineToLine(double a0,double b0,double c0,double a1,double b1,double c1)	//y=(a1*c0 - a0*c1)/(a0*b1 - a1*b0),x=(b0*c1 - b1*c0)/(a0*b1 - a1*b0) 
	{
		if(a0 * b1 - a1 * b0 == 0.0) return XVector2::zero;
		return XVector2((b0 * c1 - b1 * c0)/(a0 * b1 - a1 * b0),
			(a1 * c0 - a0 * c1)/(a0 * b1 - a1 * b0));
	...
	//两点直线公式
	XBool twoPointLine(const XVector2& lineS,const XVector2& lineE,double *a,double *b,double *c)//(x-x1)/(x2-x1)=(y-y1)/(y2-y1)
	{
	...
	//点到线段的最短距离
	double minDistancePointToLine(const XVector2& point,const XVector2& lineS,const XVector2& lineE)
	{
		//首先判断垂足是否在线段范围内
		XVector2 tempPoint = curtatePointToLine(point,lineS,lineE);
	...
	//两线段的最短距离
	//XVector2 minDistanceTwoLine(const XVector2& line1S,const XVector2& line1E,const XVector2& line2S,const XVector2& line2E)
	XVector2 minDistanceTwoLine(const XVector2&,const XVector2&,const XVector2&,const XVector2&)
	{//尚未实现
		XVector2 result(0.0,0.0);
		return result;
	}
	//使用射线法(这个方法可以扩展到任意多边形)
	XBool getIsInRect(const XVector2& p,const XVector2& p1,const XVector2& p2,const XVector2& p3,const XVector2& p4)
	{
		return getIsInRect(p.x,p.y,p1,p2,p3,p4);
	}
	XBool getIsInRect(float x,float y,const XVector2& p1,const XVector2& p2,const XVector2& p3,const XVector2& p4)
	{
		int crashSum = 0;	//射线相交的次数 
		if(p1.y != p2.y && y >= min(p1.y,p2.y) && y < max(p1.y, p2.y) && 	//不平行	
			(y - p1.y) * (p2.x - p1.x) / (p2.y - p1.y) + p1.x > x) 
			++ crashSum;
		if(p2.y != p3.y && y >= min(p2.y,p3.y) && y < max(p2.y, p3.y) &&	//不平行	
			(y - p2.y) * (p3.x - p2.x) / (p3.y - p2.y) + p2.x > x)
			++ crashSum;
		if(p3.y != p4.y && y >= min(p3.y,p4.y) && y < max(p3.y, p4.y) &&	//不平行	
			(y - p3.y) * (p4.x - p3.x) / (p4.y - p3.y) + p3.x > x)
			++ crashSum;
		if(p4.y != p1.y && y >= min(p4.y,p1.y) && y < max(p4.y, p1.y) &&	//不平行	
			(y - p4.y) * (p1.x - p4.x) / (p1.y - p4.y) + p4.x > x) 
			++ crashSum;
		//return (crashSum % 2 != 0);
		return ((crashSum & 0x01) != 0);
	}
	XBool getIsInRect(const XVector2& p,const XVector2 *pR,int pRSum)
	{
		return getIsInRect(p.x,p.y,pR,pRSum);
	}
	XBool getIsInRect(float x,float y,const XVector2 *pR,int pRSum)
	{
		...
		for(int i = 0;i < pRSum - 1;++ i)
		{
			if(pR[i].y != pR[i + 1].y && y >= min(pR[i].y,pR[i + 1].y) && y < max(pR[i].y,pR[i + 1].y) &&
				(y - pR[i].y) * (pR[i + 1].x - pR[i].x) / (pR[i + 1].y - pR[i].y) + pR[i].x > x) 
				++crashSum;
		}
		//判断闭合点
		if(pR[pRSum - 1].y != pR[0].y && y >= min(pR[pRSum - 1].y,pR[0].y) && y < max(pR[pRSum - 1].y,pR[0].y) &&
			(y - pR[pRSum - 1].y) * (pR[0].x - pR[pRSum - 1].x) / (pR[0].y - pR[pRSum - 1].y) + pR[pRSum - 1].x > x) 
			++crashSum;
		//return (crashSum % 2 != 0);
		return ((crashSum & 0x01) != 0);
	}
	...
	#define COHEN_Top 8
	inline int cohenCode(const XVector2 &p,const XRect &r)
	{//点相对于矩形的关系
		int c = 0;
	...
	int getPointsLineToRect(const XVector2 &s,const XVector2 &e,const XRect &r,XVector2 &ps,XVector2 &pe)
	{//cohenProcess
		int intCode;
		float x = 0,y = 0;
	...
	//二次方程:ax^2 + bx + c = 0
	int quadraticEquation(double a,double b,double c,std::complex<double> &z1,std::complex<double> &z2)
	{
		if(a == 0)
		{
			z2 = 0;	//无效
	...
	//XVector2 maping2D(const XVector2 *inRect,const XVector2 *outRect,const XVector2 &in)
	//{
	//	//0   1
	//	//3   2
	//	//检查数据的合法性
	...
	//	if(a == 0.0f)
	//	{
	//		if(b == 0.0f) return XVector2::zero;
	//		double yt = -c/b;
	//		double xt = cx + Kx2 * yt;
	//		if(xt == 0.0f) return XVector2::zero;
	//		xt = (bx - Kx1 * yt) / xt;
	//		if(xt >= 0.0f && xt <= 1.0f)
	//		{//合法的值
	//			XVector2 ret;
	...
	XVector2 maping2D(const XVector2 *inRect,const XVector2 *outRect,const XVector2 &in)
	{
		...
		if(a == 0.0f)
		{
			...
			if(xt >= 0.0f && xt <= 1.0f)
			{
				...
				double x2 = outRect[3].x + (outRect[2].x - outRect[3].x) * xt;
				double y2 = outRect[3].y + (outRect[2].y - outRect[3].y) * xt;
				return XVector2(x1 + (x2 - x1) * yt,y1 + (y2 - y1) * yt);
			}
		}
		double temp = b * b - 4.0f * a * c;
		if(temp >= 0.0f)
		{
			...
			if(yt0 >= 0.0f && yt0 <= 1.0f)
			{
				...
				if(xt >= 0.0f && xt <= 1.0f)
				{
					...
					double x2 = outRect[3].x + (outRect[2].x - outRect[3].x) * xt;
					double y2 = outRect[3].y + (outRect[2].y - outRect[3].y) * xt;
					return XVector2(x1 + (x2 - x1) * yt0,y1 + (y2 - y1) * yt0);
				}
			}
			if(yt1 >= 0.0f && yt1 <= 1.0f)
			{
				...
				if(xt >= 0.0f && xt <= 1.0f)
				{
					...
					double x2 = outRect[3].x + (outRect[2].x - outRect[3].x) * xt;
					double y2 = outRect[3].y + (outRect[2].y - outRect[3].y) * xt;
					return XVector2(x1 + (x2 - x1) * yt1,y1 + (y2 - y1) * yt1);
				}
			}
		}
		return XVector2::zero;
	}
	XVector2 vectorFieldLineSlerp(const XVector2 *posRect,const XVector2 *vectorRect,const XVector2 &in)
	{
	...
	float scalarFieldLineSlerp(const XVector2 *posRect,const float *scalarRect,const XVector2 &in)
	{
		//方案1：比较符合逻辑，但是计算量较大，需要多次开方
		//float l0 = in.getLength(posRect[0]);
		//float l1 = in.getLength(posRect[1]);
	...
	float getMinRate(const XVector2 &a,const XVector2 &b)
	{
		XVector2 tmp = a/b;
		return XEE_Min(tmp.x,tmp.y);
	}
#ifdef SIN_BOOK
	float sinData[628] = { 0 };
