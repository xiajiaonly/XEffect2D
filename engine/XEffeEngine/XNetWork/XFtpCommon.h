...
extern XBool analyseFTPString(const char * str,char * IP,char * path,int &port);	//从ftp字符串中解析出相关数据 例子：FTP://192.168.1.110:21/folder
//下面使用sscanf的方法分析字符串，需要保证url字符串符合URL标准
extern XBool analyseFTPStrEx(const char * str,std::string &protocol,	//协议：http、ftp等，输出为大写
							  std::string &name,		//用户名
