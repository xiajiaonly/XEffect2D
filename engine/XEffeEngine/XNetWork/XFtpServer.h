...
enum XFtpTransnationType	//FTP数据传输的模式
{
...
struct XFtpClientSockInfo
{
	...
	XFtpTransnationType transnatType;					//当前设置的传输模式
	char curPath[MAX_FILE_NAME_LENGTH];	//当前设置的相对路径
...
{
private:
	XBool m_isCreated;	//服务器是否已经建立起来
	int m_serverSock;	//服务器的套接字
	int m_clientSum;	//当前客户端的数量
	int m_allClientSum;	//所有访问的客户端的数量
	WSADATA m_wsaData;

	XBool sendData(int clientSock,const char * data,int len)	//向客户端发送数据
	{
	...
	XBool recvData(int clientSock,char * p)	//从客户端获取数据
	{
	//	if(!m_isCreated) return XFalse;	//尚未建立服务器则返回失败
	...
	XFtpUserType clientLoginProc(int clientSock);//处理客户端登陆操作，返回用户的类型，0:无效,1:普通用户,2:特殊用户
...
#endif
public:
	XBool createServer();	//建立服务器，开解accept线程，没接收一个连接，则开启一个客户端处理线程用于处理客户端请求
	void close()
