...
namespace XE{
XBool XNetServer::createServer(int port,XBool autoIP)
{
...
XBool XNetServer::sendDataSocket(char * data,int len)
{//每次发送4096个数据
...
DWORD WINAPI XNetServer::recvThread(void * pParam)
{
	XNetServer &pPar = *(XNetServer *)pParam;
	int buffLen = 4096;
	char recvBuff[4096];
	int ret = 0;
...
DWORD WINAPI XNetServer::sendThread(void * pParam)
{
	XNetServer &pPar = *(XNetServer *)pParam;
	int buffSize = 4096;
...
DWORD WINAPI XNetServer::acceptThread(void * pParam)
{//默认每次只连接一个客户端
	XNetServer &pPar = *(XNetServer *)pParam;
...
DWORD WINAPI XNetServer::boardcastThread(void * pParam)
{
	XNetServer &pPar = *(XNetServer *)pParam;
