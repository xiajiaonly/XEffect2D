...
struct XSocketData
{
    ...
    XSocketData()
        ...
        ,pData(NULL)
    {}
/*	XSocketData(const XSocketData& temp)
	{
		...
		pData = XMem::createArrayMem<char>(dataLength);
		if(pData == NULL) 
		{
			dataLength = 0;			
			return;
...
/*	XSocketData& operator = (const XSocketData& temp)
	{
...
private:
    ...
    XBool m_needMutual;                            //是否允许握手存在(2010年12月15日，服务器和客户端的这个设置必须一致)
   
    int m_serverExitFlag;                       //服务器退出标志
    int m_clientExitFlag;                       //客户端退出标志
       
    XBool dataProtectionFlag;                    //是否使用数据保护功能
    //other member
    ...
    //XSocketData m_getDataBuff[MAX_DATABUFF_DEEP];
    XSocketData *m_getDataBuff;
    int m_getDataNewP;
    int m_getDataOldP;
    int pushAMessage(XSocketData tempData);    //向队列中推入一条消息
    int popAMessage(XSocketData *tempData);    //从队列中取出一条消息
    ...
    //XSocketData m_broadcastBuff[BC_BUFF_DEEP];
    XSocketData *m_broadcastBuff;				//广播的队列，所有需要广播的数据都通过这个队列来管理，这个队列在服务器上才有，需要广播的数据会推入这个队列中，在广播线程中处理掉这些数据
    int m_broadcastNewPoint;
    int m_broadcastOldPoint;
    int pushABroadcastMessage(XSocketData tempData);    //返回推入是否成功，如果失败返回0
public:
...
private:   
    ...
    void dataBale(XSocketData *temp);                           //数据打包函数
    int dataRelieve(XSocketData *temp);                         //数据解包函数，返回解包是否成功
...
//public:    //为了测试的方便这里定以为公共方法   
    ...
    int recvDataProc(int sock,XSocketData *temp_data);          //从某socket获取数据
    int recvDataFunction(int sock,char *temp_data,int length);    //重新封装数据接收函数，使得windows下socket非阻塞
    int sendDataFunction(int sock,XSocketData temp_data,int length);    //重新封装数据发送函数，使得windows下socket非阻塞
...
inline void XSocket::resetSocketAddr(int socket)                             //设置socket可以被重用
{
    int opt = 1;
    int len = sizeof(opt);
