...
//计算时间差，单位为毫秒
double XTimer::getElapsedTimeInMicroSec()
{
#ifdef XEE_OS_WINDOWS
    if(!m_stopped)
...
double XTimer::getUpElapsedTime()
{
...
namespace XTime
{
	...
	void getTimeMs(XSystemTime &t)
	{
		...
		t.day = stime.wDay;
		//t.hour = (stime.wHour + 8) % 24;    //中国为东8时区	//不能直接加
		t.hour = stime.wHour;    //中国为东8时区	//不能直接加
	...
	//const static int dayInMonth[12] = {31,29,31,30,31,30,31,31,30,31,30,31};
	const int dayInMonth[12] = {0,31,60,91,121,152,182,213,244,274,305,335};
	bool getDateIsCorrect(const XSystemTime &t)
	{
		if(t.year < 0 ||
	...
	void getDateData(int year,int day,XSystemTime &t,int &week)
	{//这里效率可以优化的空间很大
		while(day <= 0)
		{//前几年的事情
			day += getDaysInYear(year - 1);
			-- year;
		}
		while(true)
		{//后几年的事情
...
#define MIN_LUNAR_MAP_YEAR (1900)	//农历表中最小的农历年
#define MAX_LUNAR_MAP_YEAR (2050)	//农历表中最大的农历年
	inline unsigned char getLeapMonth(unsigned int lunarYear) 
	{//确定是否存在农历的闰月 并返回闰月
		if(lunarYear < MIN_LUNAR_MAP_YEAR || lunarYear > MAX_LUNAR_MAP_YEAR) return 0;	//无效的输入参数
		return lunarInfo[lunarYear - MIN_LUNAR_MAP_YEAR]&0xf;  
	}  
	inline unsigned char getLeapMonthDay(unsigned int lunarYear)
	{//若存在闰月,返回闰月的天数,30?29
		if(lunarYear < MIN_LUNAR_MAP_YEAR || lunarYear > MAX_LUNAR_MAP_YEAR) return 0;	//无效的输入参数
		if(getLeapMonth(lunarYear))  
			return(((lunarInfo[lunarYear - MIN_LUNAR_MAP_YEAR]) & 0x10000)?30:29);  
		else  
			return(0);  
	}  
	...
	{//确定农历当月天数,30?29    
		if(lunarYear < MIN_LUNAR_MAP_YEAR || lunarYear > MAX_LUNAR_MAP_YEAR) return 0;	//无效的输入参数
		return((lunarInfo[lunarYear - MIN_LUNAR_MAP_YEAR] & (0x10000 >> lunarMonth)  )?30:29);  
	}   
	...
	{// 农历当年总天数,354?355 384 383     
		...
		for(int i = 0x8000;i > 0x8; i >>= 1)   
			sum += (lunarInfo[lunarYear - MIN_LUNAR_MAP_YEAR]&i)?1:0;		//把大月的1加进去  
	...
	{//计算1900.1.1 到  输入年月的天数    
		...
		//return total;
		//优化方案
		static const int daysTo1900 = getTotalDays(XSystemTime(1900,1,1));
		return getTotalDays(XSystemTime(solarYear,solarMonth,1)) - daysTo1900;  
	} 
	void solar2lunar(const XSystemTime &in,XSystemTime &out)	//阳历转阴历
	{
		if(in.year < MIN_LUNAR_MAP_YEAR || in.year > MAX_LUNAR_MAP_YEAR || 
	...
	void lunar2solar(const XSystemTime &in,XSystemTime &out)	//阴历转阳历（尚未实现）
	{}
}
}