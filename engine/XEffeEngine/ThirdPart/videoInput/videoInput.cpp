...
bool videoInput::setupDevice(int deviceNumber, int tconnection){
	if(deviceNumber >= VI_MAX_CAMERAS || VDList[deviceNumber]->readyToCapture) return false;

	setPhyCon(deviceNumber, tconnection);
	if(setup(deviceNumber))return true;
	return false;
}
...
bool videoInput::setupDevice(int deviceNumber, int w, int h, int tconnection){
	if(deviceNumber >= VI_MAX_CAMERAS || VDList[deviceNumber]->readyToCapture) return false;
...
void videoInput::processPixels(unsigned char * src, unsigned char * dst, int width, int height, bool bRGB, bool bFlip){
	...
	if(!bRGB)
	{
		if(bFlip)
		{
			for(int y = 0,indexD = 0,indexS = (height - 1) * widthInBytes; y < height; 
				++y, indexD += widthInBytes, indexS -= widthInBytes)
			{
				memcpy(dst + indexD, src + indexS, widthInBytes);
			}
