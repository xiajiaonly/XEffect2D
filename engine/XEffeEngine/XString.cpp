...
//sunday
_XBool _XSunday::init(const char *patt)
{
	if(patt == NULL) return XFalse;
	if(strlen(patt) <= 0) return XFalse;
	if(m_isInited)
	{//if is inited
	...
	m_patt = createArrayMem<char>(m_pattSize + 1);
...
_XBool _XSunday::search(const char *text,int &position)
{
	if(!m_isInited) return XFalse;
	if(text == NULL) return XFalse;
	if(position == NULL) return XFalse;
	if(strlen(text) <= 0) return XFalse;
	if(m_pattSize > (int)(strlen(text))) return XFalse;
...
_XBool sundaySearch(const char *text,const char *patt,int &position)
{
	if(text == NULL) return XFalse;
	if(patt == NULL) return XFalse;
	if(strlen(patt) <= 0) return XFalse;
	if(strlen(text) <= 0) return XFalse;
	if(strlen(patt) > strlen(text)) return XFalse;
...
{ 
	...
	int len = strlen(m);
	int *next = createArrayMem<int>(len);
...
_XBool kmpSearch(const char *text,const char *m,int &pos) 
{ 
	if(m == NULL || strlen(m) <= 0) return XFalse;
	if(text == NULL || strlen(text) <= 0) return XFalse;
	int lenM = strlen(m);
