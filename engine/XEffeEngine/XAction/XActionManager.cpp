	...
	for(int i = 0;i < sum;++ i)
	{
		...
		}else
		{
			...
			{//合法的文件
				...
				if(!tempAD->loadAction(tempFile->allPath,RESOURCE_LOCAL_FOLDER))	//资源位置的参数尚需要商榷
				{
...
_XBool _XActionMananger::loadAllActionDes(const char * path)	//将指定目录下的动作全部载入 
{//内部资源读取会存在问题
	if(m_isInited) return XFalse;
	char tempStr[MAX_FILE_NAME_LENGTH];
	if(path == NULL) strcpy(tempStr,ACTION_DES_PATH);	//使用默认路径
...
void _XActionMananger::release()	//资源释放
{//释放已经载入的资源
...
{//删除指定的AD
	...
	for(;it != m_actionDescriptions.end();++ it)
	{
		if(it->second == AD)
		{
			std::map<string,int>::iterator itTemp = m_actionDesMap.find(AD->getName());	//删除指定的
