...
namespace XE {
XBool XEngineLogo::init(const XVec2& position, XResPos resPos, XBool mode)
{
	...
	if (!m_logoTextRE.init("ResourcePack/pic/logo/RE.png", resPos)) return XFalse;
	...
	m_logoTextX.setScale(XVec2(m_xSize));
	//	for(int i = 0;i < 8;i++)
	//	{
	//		m_stageFlag[i] = 0;
	//	}
...
void XEngineLogo::move(float timeDelay)
{
	...
	{//初始化所有数据
		...
		m_logoTextE.setScale(XVec2::one);
		m_logoStage = 0;
	...
	{//第一个E翻身
		...
		if (m_xSize >= PI_HALF)
		{
			...
			m_logoTextRE.setScale(XVec2::zero);
			m_logoTextRE.setColor(1.0f, 1.0f, -1.0f, -1.0f);
		}
		if (!m_mode)
		{
			m_logoTextE.setScale(XVec2(1.0f, cos(m_xSize)));
		}
		else
		{
			m_logoTextE.setScale(XVec2(cos(m_xSize), 1.0f));
		}
	}
	else
		if (m_logoStage == 3)
		{//第一个E翻身
			...
			if (!m_mode)
			{
				m_logoTextRE.setScale(XVec2(1.0f, cos(m_xSize)));
			}
			else
			{
				m_logoTextRE.setScale(XVec2(cos(m_xSize), 1.0f));
			}
...
void XEngineLogo::setBackSize(const XVec2& size)		//设置背景图的尺寸
{
