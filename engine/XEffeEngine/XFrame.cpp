...
bool XFrame::loadFromFolder(const char *filename)	//从文件夹中载入资源
{
	...
	{//读取标记符	D：default, M：menutrue
		...
		if(tempFlag == 'D' || tempFlag == 'd')
		{
			for(int i =0;i < m_allFramesSum;++ i)
			{
				m_keyFramePosition[i].set(0.0f,0.0f);
			}
		}else
		{//读取偏移信息
			int temp_x;
...
XBool XFrame::init(const char *filename,XResourcePosition resoursePosition)
{
	if(m_isInited) 
	{
...
//	if(m_pShaderProc != NULL) glPushAttrib(GL_ALL_ATTRIB_BITS);
	XGL::EnableTexture2D();
	XGL::EnableBlend();
	if(!m_isFlash) XGL::SetBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
	else XGL::SetBlendFunc(GL_SRC_ALPHA,GL_ONE);
	...
	glTranslatef(x + m_keyFramePosition[keyFrameIndex].x + dw + halfW,
		y + m_keyFramePosition[keyFrameIndex].y - dh +  halfH, 0);
...
void XFrame::setAttribute(const XVector2& position,XBool loop,XBool endImmediately,int startFrame,float actionSpeed,XBool disappearAtEnd,XBool isOverturn)
{
	...
	,ypa(NULL)
{
	...
	m_keyFramePosition = XMem::createArrayMem<XVector2>(m_maxFrameSum);
}
...
XFrame& XFrame::operator = (const XFrame& temp)
{
	...
	memcpy(m_keyFramePosition,temp.m_keyFramePosition,sizeof(XVector2) * m_maxFrameSum);
