...
_XBool _XMultiText::initEx(const _XVector2& position,	//控件所在的位置
	const _XMultiTextTexture& tex,	//控件的贴图
	const char *str,const _XFontUnicode& font, float strSize,		//控件的字体及相关信息
	const _XSlider &vSlider,const _XSlider &hSlider)	//控件的其他附属依赖控件
{
...
{//将实际的字符串分配到各行去
	...
	if(m_nowStartLine == 0) m_lineStr[0].lineHeadorder = 0;//如果是从字符串开头显示，则需要初始化第一行的头位字符串开始
			...
			/*	if(lineLength >= m_lineStr[lineSum - m_nowStartLine].showStart)
				...
				if(lineOrder >= m_nowStartLine && lineOrder < m_nowStartLine + m_nowShowLineSum)
				{
					...
					m_lineStr[lineOrder - m_nowStartLine].nowText.setClipRect(m_nowStartPixels,0.0f,m_MaxLineWidthPixels + m_nowStartPixels,
						m_lineStr[lineOrder - m_nowStartLine].nowText.getTextSize().y);//设置字符串裁剪
					//这里位置也需要做相应的改变
					//（尚未完成）
...
void _XMultiText::draw()
{
	...
	if(m_withoutTex)
	{
		...
		//if(m_haveSelect != 0) m_spriteSelect.draw(&(m_mutiEditSelect->m_texture));	//显示选择的范围
		//显示当前输入的字符串
		for(int i = 0;i < m_nowShowLineSum;++ i)
		{
