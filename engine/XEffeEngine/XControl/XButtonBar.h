...
public:
	...
	const XFColor& getTextColor() const;	//获取控件字体的颜色
	...
	XBool setBtnIconPosition(const std::string& name,const XVec2& p);	//设置icon的位置
	XBool setBtnIconSize(const std::string& name, const XVec2& s);	//设置icon的缩放大小
	XBool setBtnIconSize(const std::string& name,float x){return setBtnIconSize(name,x);}
	//---------------------------------------
...
public:
	...
	virtual XBool isInRect(const XVec2& p);		//点x，y是否在物件身上，这个x，y是屏幕的绝对坐标
	virtual XVec2 getBox(int order);			//获取四个顶点的坐标，目前先不考虑旋转和缩放
	void disable();
	void enable();
