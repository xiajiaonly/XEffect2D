INLINE XBool XCalendar::canGetFocus(const XVec2& p)				//用于判断当前物件是否可以获得焦点
{
...
INLINE XBool XCalendar::isInRect(const XVec2& p)		//点x，y是否在物件身上，这个x，y是屏幕的绝对坐标
{
	if(!m_isInited) return XFalse;
	return m_curMouseRect.isInRect(p);
}
INLINE XVec2 XCalendar::getBox(int order)			//获取四个顶点的坐标，目前先不考虑旋转和缩放
{	
