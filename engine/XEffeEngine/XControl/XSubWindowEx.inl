INLINE XBool XSubWindowEx::isInRect(const XVec2& p)		//点x，y是否在物件身上，这个x，y是屏幕的绝对坐标
{
	if(!m_isInited) return XFalse;
	return m_curMouseRect.isInRect(p);
}
INLINE XVec2 XSubWindowEx::getBox(int order)			//获取四个顶点的坐标，目前先不考虑旋转和缩放
{
...
INLINE XBool XSubWindowEx::canGetFocus(const XVec2& p)				//用于判断当前物件是否可以获得焦点
{
...
INLINE XSubWindowEx::XShrinkDirection XSubWindowEx::calShrinkDir()	//计算当前可能的收缩方向
{
...
INLINE XBool XSubWindowEx::getIsInDragRect(const XVec2& p)	//判断位置是否在拖拽区域内
{
	return XRect(m_curMouseRect.getLT(),
		m_curMouseRect.getRT() + XVec2(-96.0f, 32.0f) * m_scale).isInRect(p);
}
INLINE XVec2 XSubWindowEx::correctPos(const XVec2& pos)	//更具传入的拖拽位置，修正该位置，当位置出于非法状态时，修正成合法的位置
{
	XVec2 ret = pos;
