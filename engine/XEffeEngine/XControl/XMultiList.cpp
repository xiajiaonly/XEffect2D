...
bool XMultiListSkin::loadFromFolder(const char *filename,XResourcePosition resPos)	//从文件夹中载入资源
{
...
bool XMultiListSkin::loadFromPacker(const char *filename,XResourcePosition resPos)	//从压缩包中载入资源
{
	char tempFilename[MAX_FILE_NAME_LENGTH];
	sprintf(tempFilename,"%s/%s",filename,MULTILIST_CONFIG_FILENAME);
...
bool XMultiListSkin::loadFromWeb(const char *filename,XResourcePosition resPos)		//从网页中读取资源
{
	return false;
}
...
XBool XMultiList::init(const XVector2& position,		//空间所在的位置
...
XBool XMultiList::initPlus(const char * path,		//控件的贴图
		...
		int lineSum,				//控件中的行数
		//const XMouseRightButtonMenu& mouseMenu,	//控件中使用的右键菜单(目前无效)
		XResourcePosition resoursePosition)	//水平滑动条
{
	if(m_isInited) return XFalse;	//重复初始化
	//下面是参数检查
...
XBool XMultiList::setRowWidth(int temp,int order)	//设置其中一行的宽度
{
...
XBool XMultiList::mouseProc(float x,float y,XMouseState mouseState)					//对于鼠标动作的响应函数
{
	if(!m_isInited ||	//如果没有初始化直接退出
...
void XMultiList::setScale(float x,float y)	//设置尺寸
{
	if(!m_isInited ||
		x <= 0 || y <= 0) return;
	m_scale.set(x,y);
