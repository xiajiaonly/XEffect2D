...
inline int _XMultiListBasic::getSelectIndex() const	//返回选择操作的行号
{
...
inline _XBool _XMultiListBasic::initEx(const _XVector2& position,		//上面接口的简化版本
...
inline _XBool _XMultiListBasic::canGetFocus(float x,float y)	//用于判断当前物件是否可以获得焦点
{
	if(!m_isInited ||	//如果没有初始化直接退出
		!m_isActive ||		//没有激活的控件不接收控制
		!m_isVisible ||	//如果不可见直接退出
...
inline _XBool _XMultiListBasic::isInRect(float x,float y)		//点x，y是否在物件身上，这个x，y是屏幕的绝对坐标
{
	if(!m_isInited) return XFalse;
	return m_nowMouseRect.isInRect(x,y);
}
inline _XVector2 _XMultiListBasic::getBox(int order)			//获取四个顶点的坐标，目前先不考虑旋转和缩放
{
