...
void _XControlManager::mouseProc(int x,int y,_XMouseState mouseState)
{
	...
	{//当前没有焦点控件
		...
		if(mouseState == MOUSE_MOVE)
		{
			for(int i = 0;i < m_objectSum;++ i)
			{
				if(i != m_focusOrder && m_objectType[m_objectDrawOrderMap[i]] != CTRL_OBJ_FUNCTION 
					&& m_objectType[m_objectDrawOrderMap[i]] != CTRL_OBJ_NULL) 
				{//不是焦点的物件也可以响应鼠标悬浮事件
...
void _XControlManager::insertChar(const char *ch,int len)
{
	if(m_objectType[m_objectDrawOrderMap[m_focusOrder]] != CTRL_OBJ_FUNCTION && m_objectType[m_objectDrawOrderMap[m_focusOrder]] != CTRL_OBJ_NULL) 
			((_XControlBasic *)(m_pObject[m_objectDrawOrderMap[m_focusOrder]].pObject))->insertChar(ch,len);
}
...
void _XControlManager::update(int stepTime)
{
	...
	{
		if(m_objectType[m_objectDrawOrderMap[i]] != CTRL_OBJ_NULL
			&& m_objectType[m_objectDrawOrderMap[i]] != CTRL_OBJ_FUNCTION) ((_XControlBasic *)(m_pObject[m_objectDrawOrderMap[i]].pObject))->update(stepTime);
	}
}
