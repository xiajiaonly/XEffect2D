...
public:
	...
	XFColor getTextColor() const {return m_font.getColor();}	//获取控件字体的颜色
...
public:
	XBool isInRect(float x,float y){return m_font.isInRect(x,y);}		//点x，y是否在物件身上，这个x，y是屏幕的绝对坐标
	XVector2 getBox(int order){return m_font.getBox(order);}			//获取四个顶点的坐标，目前先不考虑旋转和缩放
