...
XBool XImageList::mouseProc(const XVec2& p,XMouseState mouseState)				//对于鼠标动作的响应函数
{
	if(!m_isInited ||	//如果没有初始化直接退出
	...
	m_leftBtn.mouseProc(p,mouseState);	//左边的按钮
	m_rightBtn.mouseProc(p,mouseState);	//右边的按钮
	m_imageSld.mouseProc(p,mouseState);	//滑动条
