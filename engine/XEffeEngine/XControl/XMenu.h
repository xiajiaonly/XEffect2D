...
public:
	...
	XMenuItem * getItem(float x,float y);	//获取当前在x，y位置的物件的指针
...
public:
	...
	XVector2 m_position;	//放的位置
	XVector2 m_scaleSize;	//缩放大小
	XVector2 m_size;		//像素尺寸大小
	...
	XBool mouseProc(float x,float y,XMouseState mouseState);	//返回是否触发的事件
	//----------------------------------------------
	//++++++++++++++++++++++++++++++++++++++++++++++
	//下面加入对icon的支持
...
public:
	...
	std::string getCurChooseMenuStr(){return m_curChooseMenuStr;}	//获取当前选择的菜单完整字符串
public:
	//使用";"作为菜单项的分割符
...
public:
	...
	XBool isInRect(float x,float y) {return m_rootItem.isInRect(x,y);}		//点x，y是否在物件身上，这个x，y是屏幕的绝对坐标
