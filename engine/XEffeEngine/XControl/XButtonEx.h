...
private:
	...
	XVec2 *m_pArea;			//保存形状的点的序列
	XVec2 *m_pCurArea;		//保存形状的点的序列
...
public:
	...
	XBool init(const XVec2& position,			//控件的位置
		const XVec2 *area,int pointSum,		//用于描述按钮的响应范围的序列点，以及点的数量
		const XButtonSkin &tex,				//按钮的个中贴图信息
		const char *caption,float captionSize,const XVec2& captionPosition,	//按钮上显示的文字的相关信息
		const XFontUnicode& font);				//按钮上使用的字体
	XBool initEx(const XVec2& position,	//这是对上面接口的简化
	...
	XBool initPlus(const char *path,				//按钮的个中贴图信息
		...
		const XFontUnicode& font,XResPos resPos = RES_SYS_DEF);				//按钮上使用的字体
	XBool initWithoutSkin(const XVec2 *area,int pointSum,const char *caption,	//按钮上现实的文字的相关信息
		const XFontUnicode& font,const XVec2& captionPosition,float captionSize = 1.0f);
	XBool initWithoutSkin(const XVec2 *area,int pointSum,const char *caption)	//按钮上现实的文字的相关信息
	{
