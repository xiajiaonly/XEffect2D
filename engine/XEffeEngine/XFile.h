...
namespace XFile
{
	...
	extern char *readFileToStr(const char *filename, XResPos resPos = RES_SYS_DEF);	//从filename的文件中读取shader代码	//资源位置，0为外部资源，默认为外部资源
	...
	extern BOOL popFileOpenDlg(HWND hwnd, char* pstrFileName, char* pstrTitleName, 
		char* title, bool isOpen, const std::string&defExt = "xml",bool withFolder = false);
	//通过winAPI弹出文件打开窗口以便于人性化的选择文件名
	inline std::string getChooseFilename(char * defName, char * title, bool isOpen,
		const std::string&defExt = "xml", bool withFolder = false);
	//通过winAPI弹出对话框选择文件夹
