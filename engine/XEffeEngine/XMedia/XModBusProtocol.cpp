...
#endif
		XMem::XDELETE_ARRAY(tempData.data);	//释放数据
	}
}
...
void XModBusProtocol::sendCurData()
{
	...
	++ m_sendTime;
	unsigned short crcCheck = XMath::CRC16_Modbus(m_curSendData.data,m_curSendData.dataLen - 2);	//最后两个字节为校验位
...
#if WITH_LOG
	if(m_withLog)  showData(m_curSendData,std::string("发送:T-") + XString::toString(m_sendTime) + " ");
#endif
...
namespace XModBus{
int getModbusCMDDataSum(const XModBusData &CMD)	//从命令中解析数据数量
{
...
void XModBusProtocol::readHoldRegisters(int startAddr,int sum,int arm)	//读取多个保持寄存器
{
	if(m_modbusState.workType == TYPE_SLAVE) return;
	if(sum <= 0) return;
	unsigned char data[8];
...
void XModBusProtocol::readInputRegisters(int startAddr,int sum,int arm)	//读取多个保持寄存器
{
	if(m_modbusState.workType == TYPE_SLAVE) return;
	if(sum <= 0) return;
	unsigned char data[8];
