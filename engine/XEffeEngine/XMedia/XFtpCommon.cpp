...
_XBool analyseFTPString(const char * str,char * IP,char * path,int &port)	
{
	//方案1：字符串分析
	...
	int posx = getCharPosition(str + 6,':');
	if(pos > 0)
	{
		if(posx < pos && posx > 0)
		{//包含端口号
...
_XBool analyseFTPStrEx(const char * str,std::string &protocol,	//协议：http、ftp等，输出为大写
							  std::string &name,		//用户名
...
//	else printf("Fail");
	...
	int tPort = 0;
	char tPath[128];
	int offset = getCharPosition(str,'@');
	if(offset >= 0)
	{//路径中包含用户名和密码
		if(getCharPosition(str + offset,':') >= 0)
		{//路径中包含端口号
	...
	{//路径中不包含用户名和密码
		...
		if(getCharPosition(str + offset + 1,':') >= 0)
		{//路径中包含端口号
