...
void _XEmailSender::Char2Base64(char* pBuff64,char* pSrcBuff,int iLen)  
{  
    //1   1   1   1   1   1   1   1  
...
void _XEmailSender::AddFilePath(char * pcFilePath)//添加附件路径  
{  
    if(pcFilePath == NULL) return;
	for(int i = 0;i < m_pcFilePathList.size();++ i)  
    {  
...
void _XEmailSender::DeleteFilePath(char* pcFilePath)//删除附件路径  
{    
	for(int i = 0;i < m_pcFilePathList.size();++ i)  
    {
		if(strcmp(m_pcFilePathList[i].c_str(),pcFilePath)==0)//找到并删除它，如果没找到就算了  
        {  
			for(int j = i;j < m_pcFilePathList.size() - 1;++ j)
			{
...
int _XEmailSender::GetFileData(const char* FilePath)  
{  
	...
	m_pcFileBuff = createArrayMem<char>(len + 1);
...
bool _XEmailSender::CReateSocket(SOCKET &sock)  
{  
    ...
    if(connect(sock,(sockaddr*)&servaddr,sizeof(servaddr)) == SOCKET_ERROR)  //建立连接  
		return false;  
    return true;  
}  
bool _XEmailSender::Logon(SOCKET &sock)  
{  
...
bool _XEmailSender::SendHead(SOCKET &sock)  
{    
    memset(m_cSendBuff,0,sizeof(m_cSendBuff));  
...
bool _XEmailSender::SendFileBody(SOCKET &sock)  
{  
    const char* filePath;  
    int rt;  
    int len;  
    ...
    {  
        ...
        if(len != 0) XDELETE_ARRAY(m_pcFileBuff);
	}
	return true;
}
bool _XEmailSender::SendEnd(SOCKET &sock)  
{  
    sprintf_s(m_cSendBuff,"--INVT--\r\n.\r\n");  
    send(sock,m_cSendBuff,strlen(m_cSendBuff),0);  
...
bool _XEmailSender::SendMail(sMailInfo &smailInfo)  
{  
	m_sMailInfo = smailInfo;
    SOCKET sock;  
    if(!CReateSocket(sock))	return false;  //建立连接  
    if(!Logon(sock)) return false;  //登录邮箱  
    if(!SendHead(sock)) return false;  //发送邮件头  
    if(!SendTextBody(sock)) return false;  //发送邮件文本部分  
    if(!SendFileBody(sock)) return false;  //发送附件  
    if(!SendEnd(sock)) return false;  //结束邮件，并关闭sock  
    return true;  
}  