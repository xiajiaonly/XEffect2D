...
_XBool _XNetClient::connectToServer(const char *ip,int port)
{
...
_XBool _XNetClient::sendBoardcast()
{
...
//_XBool _XNetClient::recvBoardcast()
//{
//	unsigned char tempCheck;
//	int offset = 0;
//	int len;
...
//	return XFalse;
//}
_XBool _XNetClient::connectToServer()
{
	if(m_connectState == CONNECT_STATE_CONNECT) return XFalse;	//防止重复连接
	//连接服务器
	if(!getANewSocket(m_netSocket)) return XFalse;
	...
	{//下面建立广播线程
		...
		{//线程建立失败
			printf("Client boardcast thread create error!\n");
			return XFalse;
		}
	}
	...
	if(CreateThread(0,0,recvThread,this,0,&m_recvThread) == 0)
	{//线程建立失败
		printf("Client recv thread create error!\n");
		closesocket(m_netSocket);
		return XFalse;
	}
	if(CreateThread(0,0,sendThread,this,0,&m_sendThread) == 0)
	{//线程建立失败
		printf("Client send thread create error!\n");
		closesocket(m_netSocket);
		return XFalse;
	}
...
_XBool _XNetClient::createClient(_XBool useBroadcast,const char *ip,int port)
{
	if(m_connectState == CONNECT_STATE_CONNECT) return XFalse;	//防止重复连接
	...
	{//线程建立失败
		printf("Client connect thread create error!\n");
		closesocket(m_netSocket);
		return XFalse;
	}
...
_XBool _XNetClient::sendDataSocket(char * data,int len)
{//每次发送4096个数据
...
_XBool _XNetClient::getDataPacket(unsigned char *buff,int len)
{
