...
#endif
    m_clientConnectState = createArrayMem<char>(MAX_CLIENT);
    m_clientDelayTimes = createArrayMem<char>(MAX_CLIENT);
    m_clientSocket = createArrayMem<int>(MAX_CLIENT);
    m_getDataBuff = createArrayMem<_XSocketData>(MAX_DATABUFF_DEEP);
    m_broadcastBuff = createArrayMem<_XSocketData>(MAX_DATABUFF_DEEP);
    m_netFlags = createArrayMem<char>(MAX_NET_FLAGS);
...
#endif
{
    ...
    char *broadcastServerInfo = broadcastServerInfo = createArrayMem<char>(BCServerInfoDataLength);
...
#endif
{
    ...
    tempBroadCastData.pData = tempBroadCastData.pData = createArrayMem<char>(BC_DATA_SIZE - DATA_HEAD_SIZE);
...
int _XSocket::pushABroadcastMessage(_XSocketData tempData)
{
    ...
    m_broadcastBuff[m_broadcastNewPoint].pData = createArrayMem<char>(tempData.dataLength);        //分配内存空间
...
int _XSocket::pushAMessage(_XSocketData tempData)
{
    ...
    //拷贝数据体
    m_getDataBuff[m_getDataNewP].pData = createArrayMem<char>(tempData.dataLength * sizeof(char));
...
//  m_logFile.addLogInfo("发送数据");
    ...
    {//服务器
        ...
        if(clientOrder >= 0)
		{
			...
			char * sendData = createArrayMem<char>(length * sizeof(char));    //分配内存空间来暂存数据
...
int _XSocket::popAMessage(_XSocketData *tempData)
{
    ...
    if(m_getDataOldP != m_getDataNewP)
    {
        ...
        tempData->pData = createArrayMem<char>(tempData->dataLength * sizeof(char));
    ...
    {//服务器
        ...
        {//提取消息成功
            ...
            {//数据符合要求之后，将数据传出去
                ...
                (*p) = createArrayMem<char>(tempData.dataLength * sizeof(char));    //重新给p分配资源
                if((*p) == NULL) 
                {
...
int _XSocket::sendDataFunction(int sock,_XSocketData temp_data,int length)
{
    ...
    if(sock < 0) return 0;
    int test_error = 0;
    char *sendData = createArrayMem<char>((DATA_HEAD_SIZE + temp_data.dataLength) * sizeof(char));    //为数据发送创建存储空间
    if(sendData == NULL) return 0;
...
#endif    */
        //分配足够的内存空间来存数数据体
        temp_data->pData = createArrayMem<char>(temp_data->dataLength * sizeof(char));    //分配内存空间
        if(temp_data->pData == NULL) return 0;

        test_error = recvDataFunction(sock,temp_data->pData,temp_data->dataLength);
        if(test_error > 0 && test_error < temp_data->dataLength)
        {
            char *test_data = createArrayMem<char>(temp_data->dataLength * sizeof(char));
...
int _XSocket::broadcastData(char *pData,int length,int source)        //发送广播数据
{
    ...
    {//服务器直接将数据推入发送队列
        ...
        XDELETE_ARRAY(m_broadcastBuff[m_broadcastNewPoint].pData);
        m_broadcastBuff[m_broadcastNewPoint].pData = createArrayMem<char>(length);        //分配内存空间
        if(m_broadcastBuff[m_broadcastNewPoint].pData == NULL) return 0;    //注意这里是一个非常严重的错误，将会造成非常严重的影响
