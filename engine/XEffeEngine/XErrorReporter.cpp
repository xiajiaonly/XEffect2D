...
{//这个也不支持多线程
	...
	while (getInputEvent(tmpEvent))	//注意SDL的事件队列是不支持多线程的，只能在主线程中调用
	{
		...
		case EVENT_MOUSE:
			...
			{//空白区域按下任意鼠标键退出
				if (!m_button->isInRect(getMousePos())
					&& !m_edit->isInRect(getMousePos()))
					ret = 1;
			}
			break;
		}
		XEG.miniInputEvent(tmpEvent);
	}
	return ret;
}
...
void XErrorReporter::errorProc()
{
	...
	{//如果已经初始化，则这里是图形化界面的处理
		...
		m_screenShot->init(ERROR_REPORTER_PIC_FILENAME, RES_LOCAL_FOLDER);
...
bool XErrorReporter::init(XResPos resPos)	//初始化
{
	...
	if (!m_button->initPlus((getCommonResPos() + "ResourcePack/pic/ErrorReporter/btn").c_str(),
		" ", getDefaultFont(), 1.0f, resPos)) return false;
	...
	m_font->setString("非常抱歉,程序遇到严重错误无法继续.请按下[确定]按钮,按下[Esc]\n\
键或按下鼠标任意键退出.我们的技术人员将尽快修复这个错误!");
	m_fontDescrip->setACopy(getDefaultFont());
	m_fontDescrip->setPosition(getSceneWidth() * 0.5f - 220.0f, getSceneHeight() * 0.5f - 18.0f);
	...
	m_fontDescrip->setString("原因:");
	m_fontDescrip->setColor(XFColor::red);
	m_fontDescrip->setScale(1.2f);
	m_isInited = true;
	return true;
}
