	...
	for(int i = 0;i < 6;++ i)
	{
		if(m_userBuffFlag[i] != 0)
		{
			...
			case 1:	//准备出现
				...
				m_userBuffFlag[i] = 2;
				if(i == 1) XSoundPlayer.enableAllSlowDown();	//时间减慢的道具造成声音也减慢
				break;
			case 2:
			...
			case 7:
				...
				{
					m_userBuffFlag[i] = 0;
					if(i == 1) XSoundPlayer.disableAllSlowDown();	//时间减慢的道具造成声音也减慢
				}
				break;
			}
		}
...
void XGun::setGameTimerEnd(int mode)	//mode:0 停止 1：结束
{
	if(m_isNeedTimer != 0) 
	{
...
int XGun::canGetTools(XGemMatrix *gems,XVec2 position)	//判断是否可以获得道具
{
...
void XGun::numberOfEnergyGlare(int interval)	//能量数字较小时闪烁的动作
{
...
int XGun::crashToolsProc(XGemMatrix *gems,XGEM_TYPE type,XVec2 tempPoint1)//子弹的碰撞处理
{
	int result = 0;
	XVec2 tempPoint2;	
	//下面是道具效果
...
void XGun::followBulletProc(XGemMatrix *gems,int order)	//自动跟踪导弹的处理
{
	XVec2 tempPoint;
	XVec2 tempPoint1;
...
void XGun::crashProc(XGemMatrix *gems)//子弹的碰撞处理
{
