...
int pictureProc(_PicProcData *result,int maxPixelsOffset)
{
	//读取图片
...
int writeFileData(_PicProcData *picData,int allPicSum,int allOutputSum)
{
	FILE *fp;
...
int makePNGFileList()
{
	...
	_FileName *fileList = NULL;
	try
	{
		fileList = new _FileName[MAX_PNG_FILE_SUM];
...
int PNGProc(_PicProcData * picData,int picSum)
{
	//寻找最大的包围合
	...
	{
		if(getMinWellSize2n(picData[i].picOutSize.x) > maxW)
		{
			maxW = getMinWellSize2n(picData[i].picOutSize.x);
		}
		if(getMinWellSize2n(picData[i].picOutSize.y) > maxH)
		{
			maxH = getMinWellSize2n(picData[i].picOutSize.y);
		}
	}
	//拼合图片
	_X2DPacker tempPacker;
	//最后一个盒子没有优化
	tempPacker.m_basicBox = new _XBasicBox[MAX_BASICBOXSUM];	//定义100个盒子
	for(int i = 0;i < MAX_BASICBOXSUM;i ++)
	{
		tempPacker.m_basicBox[i].m_beUsed = 0;
		tempPacker.m_basicBox[i].m_objectBoxesOrder = new int[picSum];
	...
	tempPacker.m_objectBox = new _XObjectBox[picSum];	//定义了MAX_OBJECTSUM个物件
...
//		if(savePNG(fileName,picArm,Z_BEST_COMPRESSION) == 0) return 0;
	...
	printf("All Output picture save well!\n");
...
int textureCompare(_PicProcData *tex0,_PicProcData *tex1,int maxPixelsOffset)
{
...
int textureCompareEx(_PicProcData *tex0,_PicProcData *tex1,int maxOffset,int maxPixelsOffset,float maxDiffRate)
...
int ResourceOptimize()
{
...
#endif
		return 1;	//没有资源需要优化，直接返回优化完成
	}

	_PicProcData *picData = NULL;
	try
	{
		picData = new _PicProcData[allPicSum];
		if(picData == NULL) return 0;
	}catch(...)
	{
		return 0;
	}
	...
	//将所有的数据写入到序列帧信息文件中
	printf("Write file infomation to txt file!\n");
