						 ...
						 const _XTexture *texture,const _XTexture *textureAton)
{
	...
	try
	{
		m_atomBasic = new _XEchoParticles;
	...
	if(m_isSetEnd == 0)
	{
		...
		{//产生新的喷射粒子
			...
			{//有新的粒子
				...
				float temp = 1.0;//random(75)/100.0 + 0.5;
			...
			//		0.92,-0.001,m_textureAtom->m_w * temp,m_textureAtom->m_h * temp,64);
				...
				temp = (random(4000) / 2000.0f - 1.0f) * m_jetAngle + m_directionAngle + PI;
				m_atom[m_nowAtomPoint].m_parentParticle.m_dPosition.x = m_initSpeed * cos(temp);
				m_atom[m_nowAtomPoint].m_parentParticle.m_dPosition.y = m_initSpeed * sin(temp);
