...
int _XMultiListTexture::init(const char *normal,const char *disable,const char *select,const char *move,
							const char *title,const char *titleEnd,int resoursePosition)
{
	if(m_isInited != 0) return 0;
	//这里所有的贴图都不能为空
	if(normal == NULL || disable == NULL || select == NULL || 
		move == NULL || title == NULL || titleEnd == NULL) return 0;
...
int _XMultiList::init(const _XVector2& position,		//空间所在的位置
...
void _XMultiList::setTileStr(const char *str,int order)
{
	...
	try
	{
		tempRow->title = new char[strlen(str) + 1];
		if(tempRow->title == NULL) 
		{
			return;
		}
	}catch(...)
...
void _XMultiList::setBoxStr(const char *str,int line,int row)	//设置某一个单元格的文字
{
	...
	try
	{
		tempBox->string = new char[strlen(str) + 1];
		if(tempBox->string == NULL) 
		{
			return;
		}
	}catch(...)
...
int _XMultiList::setRowWidth(int temp,int order)	//设置其中一行的宽度
{
...
int _XMultiList::canGetFocus(float x,float y)
{
...
int _XMultiList::canLostFocus(float x,float y)
{
	return 1;
}

int _XMultiList::mouseProc(float x,float y,_XMouseState mouseState)					//对于鼠标动作的响应函数
{
	if(m_isInited == 0) return 0;		//如果没有初始化直接退出
	if(m_isActive == 0) return 0;		//没有激活的控件不接收控制
	if(m_isVisiable == 0) return 0;		//如果不可见直接退出
...
int _XMultiList::exportData(const char *fileName)	//数据导出
{//数据导出到文件中
	if(m_isInited == 0) return 0;		//如果没有初始化直接退出
	FILE *fp;
	if(fileName == NULL)
...
int _XMultiList::importData(const char *fileName)	//数据导入
{//从文件中将数据导入
	if(m_isInited == 0) return 0;		//如果没有初始化直接退出
	FILE *fp;
	if(fileName == NULL)
...
int _XMultiList::setRowSum(int rowSum)		//设置列数，超出部分填空白，或者删除
{
	if(m_isInited == 0) return 0;		//如果没有初始化直接退出
...
int _XMultiList::setLineSum(int lineSum)	//设置行数，超出的部分填空白，或者删除
{
	if(m_isInited == 0) return 0;		//如果没有初始化直接退出
...
int _XMultiList::deleteLine(int order)		//删除某一行
{
	if(m_isInited == 0) return 0;							//如果没有初始化直接退出
...
int _XMultiList::deleteRow(int order)		//删除某一列
{
	if(m_isInited == 0) return 0;							//如果没有初始化直接退出
...
int _XMultiList::insertALine(int order)		//在order处插入一行
{
	if(m_isInited == 0) return 0;
...
int _XMultiList::insertARow(int order)		//在order处插入一列
{
	if(m_isInited == 0) return 0;
	if(order < 0 || order >= m_tableRowSum) return 0;
...
int _XMultiList::moveDownLine(int order)	//将order行下移
{
	if(m_isInited == 0) return 0;
...
int _XMultiList::moveRightRow(int order)	//将order列右移
{
	if(m_isInited == 0) return 0;
...
int _XMultiList::isInRect(float x,float y)
{
	if(m_isInited == 0) return 0;
