...
protected:
	...
	int keyboardProc(int keyOrder,_XKeyState keyState);		//键盘时间的相应可以使用鼠标上下键选择menu项
	int canGetFocus(float x,float y);	//用于判断当前物件是否可以获得焦点
	int canLostFocus(float x,float y);
public:
	void setNowChoose(int temp);								//设置当前选择的菜单项

	int setACopy(const _XMouseRightButtonMenu &temp);		//建立一个与目标实体共用资源的实体

public:
	void setPosition(const _XVector2& position);	//设置菜单的位置
	...
	void setCallbackFun(void (* funChooseChange)(void *,int),void (* funChooseOver)(void *,int),void *pClass);	//设置菜单的回调函数
...
inline void _XMouseRightButtonMenu::setSize(const _XVector2& size)
{
	setSize(size.x,size.y);
}

inline void _XMouseRightButtonMenu::setPosition(const _XVector2& position)
{
