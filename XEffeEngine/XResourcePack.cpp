...
int _XResourcePack::unpackAllResource()
{
	...
	if(m_isGetOptimizeInfo == 0)
	{
		if(getOptimizeInfo() == 0)
		{
#ifdef DEBUG_MODE
	...
	for(int i = 0;i < m_fileSum;++ i)
	{
		...
		{
			if(fileNameCompare(m_targetName + m_targetOrder[j] * MAX_FILE_NAME_LENGTH,m_fileInfo[i].fileName) == 1)
			{
				needFileOrder = 1;
				break;
			}
		...
		try
		{
			tempFileData = new unsigned char[m_fileInfo[i].fileLength];
...
int _XResourcePack::getOptimizeInfo()
{
	if(m_isGetOptimizeInfo != 0) return 1;
	//从资源包中提取压缩文件信息
	...
	try
	{
		p = new unsigned char[lengthTemp];
		if(p == NULL) return 0;
	}catch(...)
	{
		return 0;
	...
	//解析一下内容
	int offset = 0;
	sscanf((char *)(p + offset),"%d,",&m_texInfoSum);
	...
	try
	{
		m_texInfo = new _XTextureInfo[m_texInfoSum];
		if(m_texInfo == NULL)
		{
			XDELETE_ARRAY(p);
			return 0;
	...
	try
	{
		m_targetOrder = new int[m_texInfoSum];
	...
	try
	{
		m_targetName = new char[m_targetTextureSum * MAX_FILE_NAME_LENGTH];
	...
	{//这里有一个回车位
		...
		{
			if(fileNameCompare(targetFileName,m_targetName + j * MAX_FILE_NAME_LENGTH) != 0)
			{//文件名相同
				m_targetOrder[i] = j;
				tempFlag = 1;
				break;
	...
	m_isGetOptimizeInfo = 1;
	return 1;
}

int _XResourcePack::releaseOptimizeInfo()
{
	if(m_isGetOptimizeInfo == 0) return 1;
...
int _XResourcePack::unpackResource(const char *fileName)
{
	...
	for(int i = 0;i < m_fileSum;++ i)
	{
		if(fileNameCompare(m_fileInfo[i].fileName,fileName) == 1)
		{
			needFileOrder = i;
			break;
		}
	...
	{//如果在文件列表中不能查找到资源则从压缩资源列表中查找资源
		...
		for(int i = 0;i < m_texInfoSum;++ i)
		{
			if(fileNameCompare(m_texInfo[i].textureName,fileName) == 1)
			{
				needFileOrder = i;
				break;
			}
		...
		{//找到资源之后提取资源并还原资源
			...
			//下面根据解压出来的信息还原资源
			if(reductionTexture(p,lengthTemp,m_texInfo[needFileOrder]) == 0)
			{
#ifdef DEBUG_MODE
	...
	try
	{
		tempFileData = new unsigned char[m_fileInfo[needFileOrder].fileLength];
		if(tempFileData == NULL) return -1;
	}catch(...)
	{
		return -1;
...
//	int tempPathDeep;
	...
	while(1)
	{
		...
		if(!firstEnterFlag) firstEnterFlag = true;
	}
	fclose(fp);
	//处理成相对路径
	if((fp = fopen(FILE_LIST_TEXT_NAME,"w")) == NULL)
...
int _XResourcePack::readNormalFileListFromResouce()
{
	...
	for(int i = 0;i < m_fileSum;++ i)
	{//读取文件列表信息
		fread(m_fileInfo[i].fileName,sizeof(m_fileInfo[i].fileName),1,fp);	
	...
	fclose(fp);
	m_haveReadedNormalFileList = true;
...
//	printf("开始取资源:%d\n",SDL_GetTicks());
	...
	try
	{
		m_fileData = new unsigned char[m_fileDataLength];
		if(m_fileData == NULL) 
		{
			fclose(fp);
			return 0;
...
//	printf("取出资源:%d\n",SDL_GetTicks());
	...
	try
	{
		fileData = new unsigned char[m_fileDataLength];
		if(fileData == NULL) 
		{
			XDELETE_ARRAY(m_fileData);
			return 0;
...
*/
	...
	{
		memcpy(m_fileInfo[i].fileName,m_fileData + m_fileDataPoint,sizeof(m_fileInfo[i].fileName));
		lockOrUnlockProc((unsigned char *)m_fileInfo[i].fileName,m_fileDataPoint,sizeof(m_fileInfo[i].fileName));
...
int _XResourcePack::writeCheckData()
{
	...
	unsigned char *fileData = NULL;
	unsigned char *fileDataHead = NULL;
	try
	{
		fileData = new unsigned char[tempLength];
		fileDataHead = new unsigned char[tempHeadLength];
	...
	try
	{
		fileData1 = new unsigned char[tempLength];
...
int _XResourcePack::checkCheckData()
{
	if(m_haveReadedFileList)
	{
		//计算MD5值
		...
		if(checkData != m_fileData[m_fileDataLength - 1]) return 0;
		return 1;
	}else
...
:m_haveReadedFileList(false)
,m_haveReadedNormalFileList(false)
,m_fileInfo(NULL)
,m_fileSum(0)
,m_headLength(0)
,m_fileData(NULL)
...
/*	for(int i = 0;i < MAX_FILE_SUM;++ i)
	...
	try
	{
		m_fileInfo = new _XResourceFileStruct[MAX_FILE_SUM];
		if(m_fileInfo == NULL) 
		{
#ifdef DEBUG_MODE
			printf("Memory Error!\n");
...
void _XResourcePack::setOutFileName(const char *temp)
{
	...
	if(m_haveReadedNormalFileList != false)
	{
		if(readNormalFileListFromResouce() != 1) 
		{
#ifdef DEBUG_MODE
...
_XResourcePack& _XResourcePack::GetInstance()
{
	static _XResourcePack m_instance;
	return m_instance;
}

void _XResourcePack::release()
{
