...
enum _XLOGBOOK_MODE
{
    XLOGBOOK_MODE_SIMPLE,    //简单模式
    XLOGBOOK_MODE_SERVER    //log服务器模式
};
...
public:
    ...
    //-------------------------------------------
private:
    _LogbookInfo *m_LogInfo;    //日志的内容
    int m_logInfoPoint;
    FILE *m_logBookFile;		//日志文件的文件指针
    char m_isInited;				//是否初始化
    int m_isReleased;            //标记日志文件中是否已经释放过
    //下面是使用log服务器的一些定义
    _XLOGBOOK_MODE m_workMode;    //日志的工作模式
    ...
    int pushAMessage(const char * message);    //向队列中推入一条信息，返回数据推入是否成功
    int popAMessage(char ** message);    //从队列中取出一条信息 return is there a message.
    //线程句柄
    ...
    int doubleStringLength(char ** message,int messageSize);    //扩大内存的宽度一倍
public:
    ...
    void addLogInfoExp(bool exp,const char *p,...);    //目前只能支持到简单的%s %d %c几种输出格式，没做复杂的处理
    //内联函数的形式
