...
public:
	...
	int init(int buffsize,int mode = 0)    //初始化，并分配内存空间
	{
		if(m_isInited != 0) return 0;    //如果已经初始化，则不能重复初始化
		if(buffsize <= 0) return 0;        //0尺寸是不行滴
		m_insertPoint = 0;                //当前的插入点
		m_popPoint = 0;                    //当前取出点
		...
		try
		{
			...
			if(m_pElement == NULL) return 0;    //内存分配失败
		}catch(...)
		{
			return 0;    //内存分配失败
		}
	...
	int popOneElement(Type &element)    //取出一个元素
	{
		if(m_isInited == 0) return 0;
		pthread_mutex_lock(m_mutex);
    ...
    int isEmpty() const   //是否为空
    {
        if(m_isInited == 0) return 1;
        if(m_pElement[m_popPoint].isEnable == 0) return 1;    //没有可以取出的数据了
        else return 0;
    }
	int isFull() const   //是否为满
	{
		if(m_isInited == 0) return 0;
		if(m_pElement[m_insertPoint].isEnable != 0) return 1;    //没有可以插入的空间了
		else return 0;
	}
};
#endif
