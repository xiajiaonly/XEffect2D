...
int _XEngineLogo::init(_XVector2 position,int resoursePosition,int mode)
{
	...
	if(m_back.init("pic/Back.png",m_resoursePosition) == 0) return 0;
...
//	m_back.setPosition(0,0);	//全窗口
	...
	if(m_logoBack.init("pic/logo/Back.png",m_resoursePosition) == 0) return 0;
	m_logoBack.setPosition(m_position);
	if(m_logoLight.init("pic/logo/Light.png",m_resoursePosition) == 0) return 0;
	m_logoLight.setPosition(m_position);
	if(m_logoMiddle.init("pic/logo/Middle.png",m_resoursePosition) == 0) return 0;
	m_logoMiddle.setPosition(m_position);
	if(m_logoTextX.init("pic/logo/X.png",m_resoursePosition) == 0) return 0;
	m_logoTextX.setPosition(m_position);
	if(m_logoTextE.init("pic/logo/E.png",m_resoursePosition) == 0) return 0;
	m_logoTextE.setPosition(m_position.x + 63,m_position.y + 1);
	m_logoTextE.setAlpha(0);
	if(m_logoTextRE.init("pic/logo/RE.png",resoursePosition) == 0) return 0;
